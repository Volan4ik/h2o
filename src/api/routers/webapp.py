...
def _secret_key(token: str) -> bytes:
    # HMAC_SHA256 with key="WebAppData"
...
def validate_init_data(init_data: str, lifetime: int = 3600) -> dict:
    ...
    if "auth_date" in parsed:
        try:
            auth_ts = int(parsed["auth_date"])  # unix
...
@router.post("/log")
async def log(payload: dict, data=Depends(tg_user_dep)):
    uid = data["user"].get("id")
    amount_ml = int(payload.get("amount_ml", 0))
    if amount_ml <= 0:
    ...
    with session() as s:
        ...
        s.add(WaterLog(user_id=u.id, ts_utc=datetime.utcnow().replace(tzinfo=timezone.utc), amount_ml=amount_ml, source="webapp"))
        s.commit()
    return {"ok": True}
